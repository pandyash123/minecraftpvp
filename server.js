...
// ------------------------------------------------------------------ game ---
/**
 * One self-contained game: its own world, players, bots, projectiles and
 * tick loop, talking to clients through `io` (a socket.io namespace). The
 * public arena is one of these; every duel gets a private one of its own
 * (see the duel section below), so nothing in a duel can leak into - or be
 * seen from - the main arena.
 */
function createGame(io, opts) {
  opts = opts || {};
  const DIFFICULTY = {
  ...
  let currentArena = WorldGen.arenaKey(opts.arena || process.env.ARENA || 'classic');
  ...
  function isWet(p) {
    ...
    for (let y = Math.floor(p.y + MC.PHYS.HEIGHT); y < W.SY; y++) {
      if (getBlock(x, y, z) !== ID.AIR) return false;
    }
    return true;
  }
  ...
  function playerHasItem(p, key) {
    ...
    if (key === 'elytra') return !!p.elytraUnlocked || hasBaseItem(p, 'elytra');
  ...
  function makePlayer(id, name, isBot, armorTier, kit, customItems, enchantOpts, swordTier, axeTier, dogArmor, trims, arrowTip) {
    ...
    // Preset kits lock both the item list and the enchantments - no custom
    ...
    return {
      ...
      slot: 0,
      health: C.MAX_HEALTH, maxHealth: C.MAX_HEALTH, absorption: 0, alive: true,
  ...
  function publicPlayer(p) {
    return {
      ...
      health: p.health, maxHealth: maxHp(p), absorption: p.absorption, alive: p.alive,
  ...
  function applyPotionEffect(p, item, t) {
    ...
    switch (item.potion) {
      ...
      case 'instantHealth':
        p.health = Math.min(maxHp(p), p.health + item.heal);
  ...
  function applyDamage(victim, amount, source, cause, kbX, kbZ, kbY) {
    ...
    if (victim.health <= 0 && cause !== 'void' && cause !== 'admin' && cause !== 'suicide' && victim.offhandKey === 'totem' && (victim.ammo.totem || 0) > 0) {
      victim.ammo.totem--;
  ...
  function kill(victim, source, cause) {
    ...
    if (source && source.id !== victim.id) {
      ...
      // Creeper Spawn Eggs: 1 or 2 per kill, at random - no Looting scaling.
      if (playerHasItem(source, 'creeper_spawn_egg')) {
        source.ammo.creeper_spawn_egg = (source.ammo.creeper_spawn_egg || 0) + (Math.random() < 0.5 ? 1 : 2);
      }
          ...
          if (hit) {
            if (pr.kind === 'arrow') {
              ...
              const dmg0 = C.TRIDENT_THROW_DAMAGE +
                (owner && hasEnchant(owner, 'trident', 'impaling') && isWet(hit.player) ? C.TRIDENT_IMPALING_BONUS_DMG : 0) +
  ...
  function stepBot(bot, dt, t) {
    ...
    if (target) {
      ...
      if (bot.weaponMode === 'full' && bot.health < maxHp(bot) * 0.35 && bot.ammo.gapple > 0 &&
        ...
        bot.health = Math.min(maxHp(bot), bot.health + gapple.heal);
      ...
      if ((bot.bonusItem === 'gapple' || bot.bonusItem === 'egap') &&
          ...
          bot.health < maxHp(bot) * 0.5 && t > (ai.nextBonusEat || 0) && Math.random() < 0.3) {
        ai.nextBonusEat = t + rand(6, 11) / Math.max(0.4, bot.skill);
        const food = ITEM_BY_KEY[bot.bonusItem];
        bot.health = Math.min(maxHp(bot), bot.health + food.heal);
      ...
      if (bot.weaponMode === 'full' && t > (ai.nextPotion || 0)) {
        let potionKey = null;
        if (bot.health < maxHp(bot) * 0.35 && playerHasItem(bot, 'pot_health') && bot.ammo.pot_health > 0) {
    ...
    if (!p.onGround) {
      ...
      if (p.fallFrom !== null && p.fallFrom !== undefined) {
        ...
        if (!glided && !floating && dist > C.FALL_SAFE && !Physics.inWater(getBlock, p.x, p.y, p.z) &&
            ...
            !MC.BOUNCY[Physics.blockUnder(getBlock, p)]) {
  ...
  function tick() {
    ...
    for (const p of players.values()) {
      ...
      if (!opts.duel && !p.bot && t - p.lastCoinTick >= MC.SHOP.COIN_IDLE_SECONDS) {
        p.lastCoinTick = t;
        awardCoins(p, MC.SHOP.COIN_IDLE_AMOUNT);
      }

      // natural regeneration
      if (p.health > 0 && p.health < maxHp(p) &&
          t - p.lastDamage > C.REGEN_DELAY && t - p.lastRegen > C.REGEN_INTERVAL) {
        p.lastRegen = t;
        p.health = Math.min(maxHp(p), p.health + 1);
      ...
      // regardless of recent damage, faster, and only for its own duration.
      {
        ...
        if (regen && p.health > 0 && p.health < maxHp(p) && t - p.lastEffectRegenTick >= C.REGEN_TICK_INTERVAL) {
          p.lastEffectRegenTick = t;
          p.health = Math.min(maxHp(p), p.health + C.REGEN_HP_PER_LEVEL_PER_TICK * regen.level);
          if (p.socket) p.socket.emit('heal', { health: p.health, absorption: p.absorption });
          io.emit('hp', { id: p.id, health: p.health, absorption: p.absorption });
        }
      }
      ...
      // Magma block: standing on one burns once a second - sneak across it, or
      // have Fire Resistance, and it's harmless (same as vanilla).
      if (p.onGround && !p.sneak && MC.MAGMA[Physics.blockUnder(getBlock, p)] &&
          !activeEffect(p, 'fireResistance', t) && t - (p.lastMagmaTick || 0) >= 1) {
        p.lastMagmaTick = t;
        applyDamage(p, C.MAGMA_DPS, null, 'magma', 0, 0, 0);
      }
  ...
  function saveSession(p) {
    ...
    opts.sessions.save(p.session, data);
  }

  // ------------------------------------------------------------- duels ---
  // Main arena: invites waiting on an answer.
  const duelRequests = new Map(); // reqId -> { from, to, kit, expires }
  let nextDuelRequestId = 1;

  /** Opens a private duel room (see duelApi in the outer server) and sends
   * everyone in `ps` to it. `bot` makes it a duel against a bot. */
  function startDuel(ps, kit, bot) {
    for (const p of ps) {
      if (!p.session) { p.socket.emit('chat', { system: true, text: 'Refresh the page to use duels.' }); return; }
    }
    for (const p of ps) saveSession(p);
    const names = ps.map(p => p.name);
    const room = opts.duels.create({ allowed: ps.map(p => p.session), kit, bot, names });
    for (const p of ps) {
      const opponent = bot ? (bot.difficulty === 'easy' ? 'an ' : 'a ') + bot.difficulty + (bot.hacks ? ' hacking' : '') + ' bot' : names.find(n => n !== p.name) || 'your opponent';
      p.socket.emit('duelStart', { ns: room.ns, kit, arena: room.arena, opponent });
    }
  }

  /** Duel rooms put the second fighter on whichever spawn point is
   * furthest from the first - opposite sides of a ring, or of the canyon. */
  function placeForDuel(p) {
    const other = [...players.values()].find(o => o !== p);
    let best = spawns[0];
    if (other) {
      let bestD = -1;
      for (const s of spawns) {
        const d = Math.hypot(s[0] - other.x, s[2] - other.z);
  ...
  /** The first death (or someone leaving) ends a duel: announce the winner,
   * freeze respawns, and send everyone back to the main arena shortly
   * after. The room closes itself a little later. */
  function endDuel(loser) {
    const duel = opts.duel;
    if (!duel || duel.over) return;
    duel.over = true;
    const winner = [...players.values()].find(p => p !== loser) || null;
    for (const p of players.values()) p.respawnAt = Infinity;
    io.emit('duelEnd', { winner: winner ? winner.name : null, loser: loser ? loser.name : null });
    setTimeout(() => io.emit('duelReturn'), 4000);
    setTimeout(() => duel.onClose(), 15000);
  }
  ...
  io.on('connection', socket => {
    ...
    socket.on('join', (data, ack) => {
      ...
      const session = data && typeof data.session === 'string' ? data.session.slice(0, 64) : null;
      const duel = opts.duel;
      // A duel room only lets in the two people it was made for.
      if (duel && (!session || !duel.allowed.has(session) || duel.over)) {
        socket.emit('duelDenied');
      ...
      if (!duel && ![...players.values()].some(p => !p.bot)) resetWorld(data && data.arena);
      // A duel locks the loadout and enchantments to the kit, whichever kit
      // it is - no custom item list, no enchant toggles.
      me = makePlayer(socket.id, name, false, data && data.armor, kit, duel ? null : data && data.customItems,
        duel ? MC.defaultEnchantOpts() : data && data.enchantOpts, data && data.swordTier, data && data.axeTier,
        data && data.dogArmor, data && data.trims, data && data.arrowTip);
      me.socket = socket;
      me.session = session;
      restoreSession(me);
      if (duel) placeForDuel(me);
      ...
      if (duel) {
        // The bot opponent turns up once its player has.
    ...
    socket.on('eat', () => {
      ...
      me.ammo[item.key]--;
      const t = now();
      me.health = Math.min(maxHp(me), me.health + item.heal);
    ...
    function command(msg) {
      ...
      // A duel is a straight fight - the only command is giving up.
      if (opts.duel) {
        if (cmd === 'kill') applyDamage(me, 999, null, 'suicide', 0, 0, 0);
        else reply('Commands are off during a duel (/kill to forfeit).');
        return;
      }
      // Secret, undocumented (not in /help) admin unlock - a one-way switch
      // for this player's current session, like the elytra unlock.
      // Until it's typed, every admin command below falls through to the
      // ordinary "Unknown command" reply, so nobody can tell they exist.
      if (cmd === '257' || cmd === 'admin257') {
        if (me.admin) { reply('Admin commands are already unlocked.'); return; }
        me.admin = true;
        reply('Admin commands unlocked: ' + ADMIN_HELP);
        return;
      }
      if (me.admin && adminCommand(cmd, parts, reply)) return;
      if (cmd === 'bots') {
        ...
        // Anyone can /kill themselves; /kill <name|@e> is admin-only and is
        // handled in adminCommand() before this is ever reached.
        applyDamage(me, 999, null, 'suicide', 0, 0, 0);
      } else if (cmd === 'spawn' && me.admin) {
        const s = pick(spawns);
    ...
    /**
     * Admin-only commands, unlocked per session by /admin257. Returns false
     * for anything it doesn't recognise (including a bare /kill), so that
     * falls through to the normal command handling.
     */
    function adminCommand(cmd, parts, reply) {
      if (cmd === 'kill') {
        const query = parts.slice(1).join(' ').trim();
        if (!query) return false;
        const targets = findPlayers(query).filter(p => p.alive);
        if (!targets.length) { reply('No living player matching "' + query + '"'); return true; }
        // A big, unarmored, un-blockable, un-totem-able hit - 'admin' isn't in
        // applyDamage's armor-reduction cause list, and the totem save skips it.
        for (const target of targets) applyDamage(target, 9999, null, 'admin', 0, 0, 0);
        reply('Killed ' + targets.length + (targets.length === 1 ? ' player.' : ' players.'));
        return true;
      }
      if (cmd === 'elytra') {
        const query = parts.slice(1).join(' ').trim();
        const targets = query ? findPlayers(query).filter(p => !p.bot) : [me];
        if (!targets.length) { reply('No player matching "' + query + '"'); return true; }
        for (const p of targets) {
          if (p.elytraUnlocked) continue;
          p.elytraUnlocked = true;
          if (p.socket) p.socket.emit('elytraUnlocked');
        }
        reply('Elytra unlocked for ' + targets.map(p => p.name).join(', ') + '.');
        return true;
      }
      if (cmd === 'effect') {
        const usage = 'Usage: /effect <name|@e> <effect|clear> [level 1-5] [seconds]  -  effects: ' + ADMIN_EFFECTS.join(', ');
        if (parts.length < 3) { reply(usage); return true; }
        const targets = findPlayers(parts[1]).filter(p => p.alive);
        if (!targets.length) { reply('No living player matching "' + parts[1] + '"'); return true; }
        const t = now();
        if (parts[2].toLowerCase() === 'clear') {
          ...
          reply('Cleared effects on ' + targets.map(p => p.name).join(', ') + '.');
          return true;
        }
        const kind = effectKind(parts[2]);
        if (!kind) { reply(usage); return true; }
        const level = clamp(parseInt(parts[3], 10) || 1, 1, 5);
        const seconds = clamp(parseFloat(parts[4]) || 30, 1, 86400);
        for (const p of targets) {
          // Set directly rather than through giveEffect() - an admin grant
          // should replace whatever's running, even a stronger dose.
          p.effects[kind] = { level, until: t + seconds };
          if (p.socket) p.socket.emit('effects', effectsSnapshot(p, t));
        }
        reply('Gave ' + kind + ' ' + level + ' for ' + seconds + 's to ' + targets.map(p => p.name).join(', ') + '.');
        return true;
      }
      if (cmd === 'maxhealth') {
        // Permanent for the rest of the session - respawn restores health to
        // this cap, not the default. Accepts an absolute value (/maxhealth Bob
        // 10), a relative one (/maxhealth Bob -4 takes away 2 hearts), or
        // 'reset'. Measured in health points: 2 = one heart, 20 = full.
        const usage = 'Usage: /maxhealth <name|@e> <1-' + C.MAX_HEALTH + ' | -N | +N | reset>  (2 = one heart)';
        if (parts.length < 3) { reply(usage); return true; }
        const targets = findPlayers(parts[1]);
      ...
      if (cmd === 'adminhelp') {
        reply('Admin commands: ' + ADMIN_HELP);
        return true;
      }
      return false;
    }

    // Duels (main arena only). A request goes to the other player as an
    // invite; accepting it opens a private duel room for the two of them.
    socket.on('duelRequest', d => {
      ...
      const target = players.get(d.target);
      if (!target || target.bot || !target.socket || target === me) { reply("That player can't be challenged."); return; }
      for (const r of duelRequests.values()) {
        if (r.from === me.id && r.to === target.id) { reply('You already challenged ' + target.name + '.'); return; }
      }
      const reqId = nextDuelRequestId++;
      duelRequests.set(reqId, { from: me.id, to: target.id, kit, expires: now() + 30 });
      target.socket.emit('duelInvite', { reqId, fromName: me.name, kit });
      reply('Duel request sent to ' + target.name + ' (' + MC.KITS[kit].name + ').');
    });
    socket.on('duelAnswer', d => {
      if (!me || !d) return;
      const req = duelRequests.get(d.reqId);
      if (!req || req.to !== me.id) return;
      duelRequests.delete(d.reqId);
      const from = players.get(req.from);
      const reply = text => socket.emit('chat', { system: true, text });
      if (!from || !from.socket) { reply("They're not here any more."); return; }
      if (!d.accept) { from.socket.emit('chat', { system: true, text: me.name + ' declined your duel.' }); return; }
      if (now() > req.expires) { reply('That duel request expired.'); return; }
      // Both picked the same kit -> that one; otherwise a coin flip.
      const mine = MC.KITS[d.kit] ? d.kit : req.kit;
      const kit = mine === req.kit ? mine : (Math.random() < 0.5 ? mine : req.kit);
      startDuel([from, me], kit, null);
    });
    ...
    socket.on('disconnect', () => {
      ...
      if (opts.duel && ![...players.values()].some(p => !p.bot)) setTimeout(() => opts.duel.onClose(), 0);
  ...
  /** Effect name as typed ("fire_resistance", "slowfalling", "Speed") -> its
   * internal key, or null. */
  function effectKind(typed) {
    const want = String(typed).toLowerCase().replace(/[^a-z]/g, '');
    return ADMIN_EFFECTS.find(k => k.toLowerCase() === want) || null;
  }

  /** Players matching a name - case-insensitive exact match first, falling
   * back to a substring match (same leniency /botdiff gives partial bot
   * names) - or '@e' for everyone, bots included. */
  function findPlayers(query) {
    ...
    return exact.length ? exact : all.filter(p => p.name.toLowerCase().includes(q));
  }
  ...
  if (!opts.duel) for (let i = 0; i < BOT_COUNT; i++) addBot(defaultDifficulty);
  const tickTimer = setInterval(tick, 1000 / TICK_HZ);
...
app.get('/health', (req, res) => res.json({
  ok: true, players: main.players.size, bots: [...main.players.values()].filter(p => p.bot).length, seed: SEED, arena: main.arena,
  duels: duels.size
}));
...
app.post('/reset', (req, res) => {
  main.resetWorld();
  main.emit('worldReset', { seed: SEED, arena: main.arena });
  main.emit('chat', { system: true, text: 'The terrain was reset.' });
  res.json({ ok: true });
});
...
const sessions = {
  load(id) {
    const s = id && sessionStore.get(id);
    if (!s) return null;
    s.savedAt = Date.now();
    return s;
  },
...
// ---------------------------------------------------------------- duels ---
// A duel is a whole private game (createGame) on its own socket.io
// namespace, with its own world - only the two duellists (or one player and
// their bot) can join it. It shuts itself down once the duel is over.
const duels = new Map(); // id -> { game, nsName, timer }
let nextDuelId = 1;

const duelApi = {
  /**
   * Starts a private duel room. `allowed` is the session ids that may
   * join; `bot` ({ difficulty, hacks }) adds a bot opponent instead of a
   * second player. Returns the namespace clients should connect to.
   */
  create({ allowed, kit, bot, names }) {
    const id = (nextDuelId++) + '-' + Math.random().toString(36).slice(2, 8);
    const nsName = '/duel-' + id;
    const arena = WorldGen.ARENA_KEYS[Math.floor(Math.random() * WorldGen.ARENA_KEYS.length)];
    const ns = io.of(nsName);
    const entry = { nsName };
    const game = createGame(ns, {
      arena, bots: 0, sessions,
      duel: { id, allowed: new Set(allowed), kit, bot: bot || null, names: names || [], onClose: () => closeDuel(id) }
    });
    entry.game = game;
    // Nobody turned up (closed the tab, lost connection) - don't leave an
    // empty world ticking forever.
    entry.timer = setTimeout(() => { if (!game.players.size || ![...game.players.values()].some(p => !p.bot)) closeDuel(id); }, 30 * 1000);
    duels.set(id, entry);
    return { ns: nsName, arena, kit };
  }
};

function closeDuel(id) {
  const entry = duels.get(id);
  if (!entry) return;
  duels.delete(id);
  clearTimeout(entry.timer);
  entry.game.stop();
  const ns = io.of(entry.nsName);
  ns.disconnectSockets(true);
  ns.removeAllListeners();
  io._nsps.delete(entry.nsName);
}

// The public arena everyone lands in from the menu.
const main = createGame(io, { sessions, duels: duelApi });
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  seed=' + SEED + '  world=' + W.SX + 'x' + W.SY + 'x' + W.SZ + '  bots=' + BOT_COUNT + '  arena=' + main.arena);
});
